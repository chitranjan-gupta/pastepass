document.addEventListener("DOMContentLoaded", function() {
  ...
  browser.storage.local.get(['clickpaste', 'savedText', 'clipboard_allow'], function(result) {
  ...
  saveBtn.addEventListener("click", function() {
    ...
    browser.storage.local.set({ savedText: text }, function () {
  ...
  speedBtn.addEventListener('click', async function(){
    try {
      browser.storage.local.get(['speed_input'], async function(result){
        console.log(result)
        const [tab] = await browser.tabs.query({active: true, lastFocusedWindow: true});
        const response = await browser.tabs.sendMessage(tab.id, { action: "speed_type", data: textInput.value, selector: result.speed_input || "input", speed: speed.value });
      });
    } catch (err) {
      console.log(err);
    }
  ...
  clickpaste.addEventListener('change', function(){
    browser.storage.local.set({ clickpaste: clickpaste.checked }, function() {
      console.log(`Checkbox state saved: ${clickpaste.checked}`);
    });
  })

  clipboard_allow.addEventListener('change', function(){
    browser.storage.local.set({ clipboard_allow: clipboard_allow.checked }, function() {
