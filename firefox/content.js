browser.paste_pass_clicked_element = null;
  ...
  function(request, sender, sendResponse) {
  	...
  	if(request.action === "speed_type"){
  		const selector = JSON.parse(request.selector);
			const inputElement = browser.paste_pass_clicked_element || document.querySelector(typeof selector === "object" ? `${selector.tagName.toLowerCase()}${selector.type.length > 0 ? `[type=${selector.type}]`: ''}${selector.id.length > 0 ? `[id=${selector.id}]`: ''}${selector.name.length > 0 ? `[name=${selector.name}]`: ''}${selector.className.length > 0 ? `[class=${selector.className}]`: ''}` : 'input');
			if(inputElement){
				simulateTyping(inputElement, request.data, Number(request.speed) || 100);	
			}
  	}
  	// sendResponse("jess")
  	// browser.runtime.sendMessage("sksk")
  }
);
...
function doSomething(event){
	if(event.target){
		if(event.target instanceof HTMLInputElement || event.target instanceof HTMLTextAreaElement){
			browser.paste_pass_clicked_element = event.target;
			...
			browser.storage.local.set({ speed_input: speed_input }, function(){
